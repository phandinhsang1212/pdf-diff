		...
		var AddFile = function(uploads, e, data) {
			...
			var RemoveFile = function(e) {
				...
				else
				{
					...
					if (settings.deleted)  {
						// data.originalFiles.splice(data.originalFiles.indexOf(data.files[0]), 1)
						// settings.deleted.call(inforow, e, data.originalFiles);
						res.splice(res.indexOf(data.files[0]), 1)
						settings.deleted.call(inforow, e, res);
					}
					
					//console.log(data.files[0]);
					// if (settings.deleted)  {
					// 	//console.log(filename);
					// 		settings.deleted.call(inforow, e, data.originalFiles.filter(function(ele){ 
					// 			console.log(ele != data.files[0]);
					// 		return ele != data.files[0]; 
					// 	}));
					// }
			...
			if (settings.edit && !data.ff_info.errors.length)
			{
				inforow.find('.ff_fileupload_filename').text(filename);
			}
			else
			{
			...
			uploads.empty().append(inforow);

			if (settings.added){
				res.push(data.files[0]);
				settings.added.call(inforow, e, [data.files[0]]);
				//settings.added.call(inforow, e, data);
				// settings.added.call(inforow, e, res);
			}  
